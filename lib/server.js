...
function serveExpress (port, path, cb) {
    ...
    app.get('/project/*', function (req, res) {
        ...
        // the root in win is not lost since it is not a "/", so no need to restore
        var rootPath = (process.platform == "win32") ? "" : "/";
        var file = rootPath + nodepath.normalize(decodeURIComponent(splat));
